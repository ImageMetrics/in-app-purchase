...
module.exports.getPurchaseData = function (purchase) {
	...
	if (purchase.receipt.in_app) {
		...
		for (var i = 0, len = purchase.receipt.in_app.length; i < len; i++) {
			...
			data.push({
				...
				originalPurchaseDate: item.original_purchase_date_ms,
	...
	data.push({
		...
		purchaseDate: purchase.receipt.purchase_date_ms,
		originalPurchaseDate: purchase.receipt.original_purchase_date_ms,
